    ...
    def download_image(image_url, image_file_name, destination_blob_name, logger):
        ...
        try:
            ...
            if err.code == 404:
                ...
                logger.error(f"ray_worker_node_id:{ray_worker_node_id} Unhandled HTTPError exception")
        except urllib.error.URLError:
            logger.error(f"ray_worker_node_id:{ray_worker_node_id} URLError exception")
        except:
            logger.error(f"ray_worker_node_id:{ray_worker_node_id} Unhandled exception")
        return False
...
def run_remote():
    ...
    logger.debug(f"Data Preperation finished in {duration} seconds")
